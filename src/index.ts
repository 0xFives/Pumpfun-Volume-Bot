...
const distributeSol = async (connection: Connection, mainKp: Keypair, distritbutionNum: number) => {
  ...
  try {
    ...
    logger.info('🚀 mainWalletSolBalance:', mainWalletSolBalance / LAMPORTS_PER_SOL);
    ...
    logger.info(`Success in distribution: https://solscan.io/tx/${distributeTxSignature ?? ''}`);
    subWallets.forEach((subWallet, index) => {
      logger.info(`Wallet ${index + 1} Pubkey: ${subWallet.kp.publicKey.toBase58()}, Balance: ${subWallet.buyAmount}`);
    });
    logger.info(`-------------------------------------------------------------------------`);
    ...
    logger.error('Error in Sol Distribution:', error);
...
const buy = async (wallet: Keypair, baseMint: PublicKey, buyAmount: number) => {
  try {
    ...
    if (simulateBuyTx.value.err) {
      logger.error(`Simulation Result of Buy Transaction on ${wallet.publicKey.toBase58()}: ${simulateBuyTx}`);
    ...
    if (txSignature) {
      logger.info(`Success in buy transaction: https://solscan.io/tx/${txSignature}`);
      return txSignature;
    } else {
      throw new Error('Error in confirm transaction:');
    }
  } catch (error) {
    logger.error(`Error in Pumpfun token buy transaction: ${error}`);
...
const sell = async (baseMint: PublicKey, wallet: Keypair) => {
  try {
    ...
    if (simulateSellTx.value.err) {
      logger.error(`Simulation Result of Sell Transaction on ${wallet.publicKey.toBase58()}: ${simulateSellTx}`);
    ...
    if (txSignature) {
      logger.info(`Success in sell transaction: https://solscan.io/tx/${txSignature}`);
      return txSignature;
    } else {
      throw new Error('Error in confirm transaction:');
    }
  } catch (error) {
    logger.error(`Error in Pumpfun token sell transaction: ${error}`);
