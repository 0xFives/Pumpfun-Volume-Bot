  ...
  try {
    ...
    try {
      if (fs.existsSync(filePath)) {
        ...
        logger.info(`File ${filePath} deleted and create new file.`);
      }
...
export function editJson(newData: Partial<Data>, filename: string = 'data.json'): void {
  if (!newData.pubkey) {
    logger.warn('Pubkey is not prvided as an argument');
    return;
  }
  ...
  if (index !== -1) {
    ...
    logger.error(`Pubkey ${newData.pubkey} does not exist.`);
  }
}
