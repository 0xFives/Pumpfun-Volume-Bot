...
export const makeBuyPumpfunTokenTx = async (payer: Keypair, mintPublickey: PublicKey, buyAmount: number) => {
  try {
    ...
    logger.error('Error while making buy transaction in pumpfun', error);
...
export const makeSellPumpfunTokenTx = async (payer: Keypair, mintPublickey: PublicKey) => {
  try {
    ...
    logger.info(`SellAmount: ${BigInt(balance.value.amount) / BigInt(2) / BigInt(100000)}`);
    ...
    logger.error('Error while making sell transaction in pumpfun', error);
