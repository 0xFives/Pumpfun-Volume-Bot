  ...
  wallets.map(async (wallet: Keypair, i: number) => {
    try {
      ...
      while (true) {
        ...
        if (solBalance <= 0) {
          logger.info(`${walletPublicKey.toBase58()} has no SOL`);
          break;
        }

        logger.info(`${walletPublicKey.toBase58()} has ${solBalance / LAMPORTS_PER_SOL} SOL`);
        ...
        try {
          ...
          if (tokenBalance.uiAmount && tokenBalance.uiAmount > 0) {
            logger.info(`${wallet.publicKey.toBase58()} has ${tokenBalance.uiAmount} tokens after sell`);
            try {
              ...
              logger.info('Token ATA exists on main wallet');
            } catch (error) {
              ixs.push(
                createAssociatedTokenAccountIdempotentInstruction(
                  mainKp.publicKey,
            ...
            logger.info('Token Transfer Instruction');
          ...
          logger.info(`Close Token Ata instruction`);
          ...
          logger.warn(`Token ATA does not exist on ${wallet.publicKey.toBase58()}`);
        }
        // let i = 0;
        // while (true) {
        ...
        // if (accountInfo) {
        logger.info('Sol Tranfer Instruction');
        ixs.push(
        ...
        if (ixs.length) {
          ...
          while (retry < 5) {
            try {
              ...
              if (simResult.value.err) {
                logger.error('Simulation result error:', simResult);
              ...
              logger.info(`Gathered SOL from ${wallet.publicKey.toBase58()} : https://solscan.io/tx/${sig}`);
              break;
            } catch (error) {
              retry++;
              await sleep(1000);
      ...
      logger.error('transaction error while gathering', error);
      return;
    }
  });
};
